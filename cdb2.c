...
Schema create_default_schema(void) {
    Schema s;
    memset(&s, 0, sizeof(Schema));
    strncpy(s.table_name, "employees", MAX_NAME_LEN - 1);
    schema_add_column(&s, "id", DATA_TYPE_INT, 0, true);
    schema_add_column(&s, "name", DATA_TYPE_VARCHAR, 32, false);
    schema_add_column(&s, "salary", DATA_TYPE_INT, 0, false);
    schema_add_column(&s, "department", DATA_TYPE_VARCHAR, 32, false);
    schema_add_column(&s, "city", DATA_TYPE_VARCHAR, 32, false);
    return s;
}
...
// ============================================================================
// BusTub-style Buffer Pool Manager & Replacement Policy
// ============================================================================
...
typedef struct {
    ...
    Frame pool[BUFFER_POOL_SIZE];
    int page_table[TABLE_MAX_PAGES]; // Maps page_num -> frame_id (-1 if not cached)
    uint64_t lru_clock;
} BufferPoolManager;

BufferPoolManager* bpm_open(const char* filename) {
    ...
    bpm->file_descriptor = fd;
    bpm->file_length = (uint32_t)file_length;
    bpm->num_pages = bpm->file_length / PAGE_SIZE;
    bpm->lru_clock = 0;

    if (bpm->file_length % PAGE_SIZE != 0) {
    ...
    for (int i = 0; i < BUFFER_POOL_SIZE; i++) {
        bpm->pool[i].page_num = INVALID_PAGE_NUM;
        bpm->pool[i].pin_count = 0;
        bpm->pool[i].is_dirty = false;
        bpm->pool[i].lru_access_time = 0;
        bpm->pool[i].data = malloc(PAGE_SIZE);
        memset(bpm->pool[i].data, 0, PAGE_SIZE);
    }

    return bpm;
}

void bpm_flush_frame(BufferPoolManager* bpm, int frame_id) {
    Frame* frame = &bpm->pool[frame_id];
    if (frame->page_num != INVALID_PAGE_NUM && frame->is_dirty) {
        lseek(bpm->file_descriptor, frame->page_num * PAGE_SIZE, SEEK_SET);
        ssize_t bytes_written = write(bpm->file_descriptor, frame->data, PAGE_SIZE);
        if (bytes_written == -1) {
            printf("Error writing page %u: %d\n", frame->page_num, errno);
            exit(EXIT_FAILURE);
        }
...
void* bpm_fetch_page(BufferPoolManager* bpm, uint32_t page_num) {
    ...
    int frame_id = bpm->page_table[page_num];
    if (frame_id != -1) {
        Frame* frame = &bpm->pool[frame_id];
        frame->pin_count++;
        frame->lru_access_time = ++bpm->lru_clock;
        return frame->data;
    }

    // 2. Look for free frame or evict using LRU policy
    int target_frame = -1;
    for (int i = 0; i < BUFFER_POOL_SIZE; i++) {
        if (bpm->pool[i].page_num == INVALID_PAGE_NUM) {
            target_frame = i;
            break;
        }
    }

    if (target_frame == -1) { // Eviction required
        uint64_t min_lru = UINT64_MAX;
        for (int i = 0; i < BUFFER_POOL_SIZE; i++) {
            if (bpm->pool[i].pin_count == 0 && bpm->pool[i].lru_access_time < min_lru) {
                min_lru = bpm->pool[i].lru_access_time;
                target_frame = i;
            }
        }

        if (target_frame == -1) {
            printf("Buffer Pool Manager Error: All buffer frames are pinned! Cannot evict.\n");
            exit(EXIT_FAILURE);
        }

        // Flush dirty page
        bpm_flush_frame(bpm, target_frame);
        bpm->page_table[bpm->pool[target_frame].page_num] = -1;
    }

    // 3. Load page into target frame
    Frame* frame = &bpm->pool[target_frame];
    ...
    if (page_num * PAGE_SIZE < bpm->file_length) {
        lseek(bpm->file_descriptor, page_num * PAGE_SIZE, SEEK_SET);
        ssize_t bytes_read = read(bpm->file_descriptor, frame->data, PAGE_SIZE);
        if (bytes_read == -1) {
            printf("Error reading page %u: %d\n", page_num, errno);
            exit(EXIT_FAILURE);
        }
    }

    if (page_num >= bpm->num_pages) {
        bpm->num_pages = page_num + 1;
    }

    bpm->page_table[page_num] = target_frame;
    return frame->data;
}

void bpm_unpin_page(BufferPoolManager* bpm, uint32_t page_num, bool is_dirty) {
    int frame_id = bpm->page_table[page_num];
...
void bpm_flush_all(BufferPoolManager* bpm) {
    for (int i = 0; i < BUFFER_POOL_SIZE; i++) {
        bpm_flush_frame(bpm, i);
    }
}

void bpm_close(BufferPoolManager* bpm) {
    bpm_flush_all(bpm);
    for (int i = 0; i < BUFFER_POOL_SIZE; i++) {
        free(bpm->pool[i].data);
    }
    if (bpm->file_descriptor != -1) {
        close(bpm->file_descriptor);
    }
    free(bpm);
}

void print_bpm_status(const BufferPoolManager* bpm) {
    printf("=== BusTub Buffer Pool Manager State ===\n");
    printf("Total Pages: %u | Pool Capacity: %d frames\n", bpm->num_pages, BUFFER_POOL_SIZE);
    printf("Frame | Page ID | Pin Count | Dirty | LRU Clock\n");
    printf("------+---------+-----------+-------+----------\n");
    for (int i = 0; i < BUFFER_POOL_SIZE; i++) {
        const Frame* f = &bpm->pool[i];
        if (f->page_num == INVALID_PAGE_NUM) {
            printf("  %2d  |  EMPTY  |     -     |   -   |    -\n", i);
        } else {
            printf("  %2d  |  %5u  |    %2u     |   %c   | %llu\n",
                   i, f->page_num, f->pin_count, f->is_dirty ? 'Y' : 'N', (unsigned long long)f->lru_access_time);
        }
    }
}
...
Table* table_open(const char* filename) {
    ...
    if (table->bpm->num_pages == 0) {
        table->schema = create_default_schema();
        void* root_node = bpm_fetch_page(table->bpm, 0);
        initialize_leaf_node(root_node);
        set_node_root(root_node, true);
        bpm_unpin_page(table->bpm, 0, true);
    } else {
        table->schema = create_default_schema();
    }
    return table;
...
uint32_t get_node_max_key(Table* table, void* node) {
    ...
    void* right_child = bpm_fetch_page(table->bpm, right_child_page);
    uint32_t max_key = get_node_max_key(table, right_child);
    bpm_unpin_page(table->bpm, right_child_page, false);
    return max_key;
}

Cursor* leaf_node_find(Table* table, uint32_t page_num, uint32_t key) {
    void* node = bpm_fetch_page(table->bpm, page_num);
    ...
    while (one_past_max_index != min_index) {
        ...
        if (key == key_at_index) {
            ...
            bpm_unpin_page(table->bpm, page_num, false);
            return cursor;
        }
        if (key < key_at_index) {
...
Cursor* internal_node_find(Table* table, uint32_t page_num, uint32_t key) {
    ...
    void* child = bpm_fetch_page(table->bpm, child_num);
    NodeType type = get_node_type(child);
    bpm_unpin_page(table->bpm, child_num, false);
...
Cursor* table_find(Table* table, uint32_t key) {
    void* root_node = bpm_fetch_page(table->bpm, table->root_page_num);
    NodeType root_type = get_node_type(root_node);
    bpm_unpin_page(table->bpm, table->root_page_num, false);

    if (root_type == NODE_LEAF) {
...
Cursor* table_start(Table* table) {
    ...
    void* node = bpm_fetch_page(table->bpm, cursor->page_num);
    uint32_t num_cells = *leaf_node_num_cells(node);
    cursor->end_of_table = (num_cells == 0);
    bpm_unpin_page(table->bpm, cursor->page_num, false);
    return cursor;
}

void* cursor_value(Cursor* cursor) {
    void* page = bpm_fetch_page(cursor->table->bpm, cursor->page_num);
    void* val = leaf_node_value(page, cursor->cell_num, cursor->table->schema.row_size);
    bpm_unpin_page(cursor->table->bpm, cursor->page_num, false);
    return val;
}

void cursor_advance(Cursor* cursor) {
    void* node = bpm_fetch_page(cursor->table->bpm, cursor->page_num);
    cursor->cell_num += 1;
...
void leaf_node_delete(Cursor* cursor) {
    ...
    *(leaf_node_num_cells(node)) -= 1;
    bpm_unpin_page(cursor->table->bpm, cursor->page_num, true);
}

uint32_t get_unused_page_num(Table* table) {
    return table->bpm->num_pages;
}

void create_new_root(Table* table, uint32_t right_child_page_num) {
    void* root = bpm_fetch_page(table->bpm, table->root_page_num);
    void* right_child = bpm_fetch_page(table->bpm, right_child_page_num);
    uint32_t left_child_page_num = get_unused_page_num(table);
    void* left_child = bpm_fetch_page(table->bpm, left_child_page_num);
    ...
    if (get_node_type(left_child) == NODE_INTERNAL) {
        ...
        for (uint32_t i = 0; i < *internal_node_num_keys(left_child); i++) {
            uint32_t child_pg = *internal_node_child(left_child, i);
            child = bpm_fetch_page(table->bpm, child_pg);
            *node_parent(child) = left_child_page_num;
            bpm_unpin_page(table->bpm, child_pg, true);
        }
        uint32_t right_pg = *internal_node_right_child(left_child);
        child = bpm_fetch_page(table->bpm, right_pg);
        *node_parent(child) = left_child_page_num;
        bpm_unpin_page(table->bpm, right_pg, true);
    }
    ...
    *node_parent(right_child) = table->root_page_num;
    ...
    bpm_unpin_page(table->bpm, table->root_page_num, true);
    bpm_unpin_page(table->bpm, right_child_page_num, true);
    bpm_unpin_page(table->bpm, left_child_page_num, true);
}
...
void internal_node_insert(Table* table, uint32_t parent_page_num, uint32_t child_page_num) {
    void* parent = bpm_fetch_page(table->bpm, parent_page_num);
    void* child = bpm_fetch_page(table->bpm, child_page_num);
    uint32_t child_max_key = get_node_max_key(table, child);
    uint32_t index = internal_node_find_child(parent, child_max_key);

    uint32_t original_num_keys = *internal_node_num_keys(parent);
    if (original_num_keys >= INTERNAL_NODE_MAX_KEYS) {
        bpm_unpin_page(table->bpm, parent_page_num, false);
        bpm_unpin_page(table->bpm, child_page_num, false);
    ...
    if (right_child_page_num == INVALID_PAGE_NUM) {
        *internal_node_right_child(parent) = child_page_num;
        bpm_unpin_page(table->bpm, parent_page_num, true);
        bpm_unpin_page(table->bpm, child_page_num, false);
        return;
    }
    ...
    *internal_node_num_keys(parent) = original_num_keys + 1;
    ...
    bpm_unpin_page(table->bpm, right_child_page_num, false);
    bpm_unpin_page(table->bpm, parent_page_num, true);
    bpm_unpin_page(table->bpm, child_page_num, false);
}
...
void internal_node_split_and_insert(Table* table, uint32_t parent_page_num, uint32_t child_page_num) {
    ...
    void* old_node = bpm_fetch_page(table->bpm, parent_page_num);
    ...
    if (splitting_root) {
        bpm_unpin_page(table->bpm, old_page_num, false);
        bpm_unpin_page(table->bpm, child_page_num, false);
        create_new_root(table, new_page_num);
        parent = bpm_fetch_page(table->bpm, table->root_page_num);
        old_page_num = *internal_node_child(parent, 0);
        old_node = bpm_fetch_page(table->bpm, old_page_num);
    } else {
        parent = bpm_fetch_page(table->bpm, *node_parent(old_node));
        new_node = bpm_fetch_page(table->bpm, new_page_num);
        initialize_internal_node(new_node);
    }

    uint32_t* old_num_keys = internal_node_num_keys(old_node);
    uint32_t cur_page_num = *internal_node_right_child(old_node);
    void* cur = bpm_fetch_page(table->bpm, cur_page_num);

    internal_node_insert(table, new_page_num, cur_page_num);
    *node_parent(cur) = new_page_num;
    *internal_node_right_child(old_node) = INVALID_PAGE_NUM;
    bpm_unpin_page(table->bpm, cur_page_num, true);

    for (int i = INTERNAL_NODE_MAX_KEYS - 1; i > (int)(INTERNAL_NODE_MAX_KEYS / 2); i--) {
        cur_page_num = *internal_node_child(old_node, i);
        cur = bpm_fetch_page(table->bpm, cur_page_num);
        internal_node_insert(table, new_page_num, cur_page_num);
        *node_parent(cur) = new_page_num;
        (*old_num_keys)--;
        bpm_unpin_page(table->bpm, cur_page_num, true);
    ...
    *internal_node_right_child(old_node) = *internal_node_child(old_node, *old_num_keys - 1);
    ...
    cur = bpm_fetch_page(table->bpm, child_page_num);
    *node_parent(cur) = destination_page_num;
    bpm_unpin_page(table->bpm, child_page_num, true);
    ...
    if (!splitting_root) {
        ...
        *node_parent(new_node) = *node_parent(old_node);
        bpm_unpin_page(table->bpm, new_page_num, true);
        bpm_unpin_page(table->bpm, *node_parent(old_node), true);
    } else {
        bpm_unpin_page(table->bpm, table->root_page_num, true);
    }

    bpm_unpin_page(table->bpm, old_page_num, true);
    bpm_unpin_page(table->bpm, child_page_num, false);
}

void leaf_node_split_and_insert(Cursor* cursor, uint32_t key, DynamicRow* value) {
    Table* table = cursor->table;
    void* old_node = bpm_fetch_page(table->bpm, cursor->page_num);
    uint32_t old_max = get_node_max_key(table, old_node);
    uint32_t new_page_num = get_unused_page_num(table);
    void* new_node = bpm_fetch_page(table->bpm, new_page_num);
    ...
    *(leaf_node_num_cells(new_node)) = right_split_count;
    ...
    bool is_root = is_node_root(old_node);
    bpm_unpin_page(table->bpm, cursor->page_num, true);
    bpm_unpin_page(table->bpm, new_page_num, true);

    if (is_root) {
        create_new_root(table, new_page_num);
    } else {
        old_node = bpm_fetch_page(table->bpm, cursor->page_num);
...
ExecuteResult execute_insert(Statement* statement, Table* table) {
    if (table->bpm->num_pages + INSERT_PAGE_SAFETY_MARGIN > TABLE_MAX_PAGES) {
...
ExecuteResult execute_delete(Statement* statement, Table* table) {
    ...
    for (uint32_t i = 0; i < delete_count; i++) {
        ...
        void* node = bpm_fetch_page(table->bpm, c->page_num);
        uint32_t num_cells = *leaf_node_num_cells(node);
        if (c->cell_num < num_cells && *leaf_node_key(node, c->cell_num, table->schema.row_size) == key) {
            bpm_unpin_page(table->bpm, c->page_num, false);
            leaf_node_delete(c);
        } else {
            bpm_unpin_page(table->bpm, c->page_num, false);
        }
...
ExecuteResult execute_begin(Table* table) {
    ...
    table->tx_original_num_pages = table->bpm->num_pages;

    for (uint32_t i = 0; i < table->bpm->num_pages; i++) {
        void* page_ptr = bpm_fetch_page(table->bpm, i);
        table->tx_backup[i] = malloc(PAGE_SIZE);
        memcpy(table->tx_backup[i], page_ptr, PAGE_SIZE);
        bpm_unpin_page(table->bpm, i, false);
    }
    return EXECUTE_SUCCESS;
}
...
ExecuteResult execute_rollback(Table* table) {
    ...
    for (uint32_t i = 0; i < table->tx_original_num_pages; i++) {
        if (table->tx_backup[i] != NULL) {
            void* page_ptr = bpm_fetch_page(table->bpm, i);
            memcpy(page_ptr, table->tx_backup[i], PAGE_SIZE);
            bpm_unpin_page(table->bpm, i, true);
            free(table->tx_backup[i]);
            table->tx_backup[i] = NULL;
        }
    }
    table->bpm->num_pages = table->tx_original_num_pages;
    table->in_transaction = false;
    return EXECUTE_SUCCESS;
}
...
void print_help(void) {
    printf("SQL Commands:\n"
           ...
           "  \\b or .bpm       print BusTub Buffer Pool Manager status\n"
...
MetaCommandResult do_meta_command(const char* input, Table* table) {
    ...
    } else if (strcmp(input, ".bpm") == 0 || strcmp(input, "\\b") == 0) {
        print_bpm_status(table->bpm);
        return META_COMMAND_SUCCESS;
