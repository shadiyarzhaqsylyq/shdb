...
CREATE TABLE posts (id INT PRIMARY KEY, title VARCHAR(64), user_id INT);

INSERT INTO users VALUES (1, 'alice');
INSERT INTO posts VALUES (10, 'Hello', 1);

SELECT * FROM users;
SELECT * FROM posts WHERE id > 5 LIMIT 10;

.schema          -- shows both tables
DROP TABLE posts;
SELECT * FROM posts;   -- Error: table "posts" does not exist.
...
// On-disk compact entry (fits easily in the meta page)
typedef struct CompactTableMeta {
    uint8_t used;
    char name[MAX_NAME_LEN];
    uint32_t root_page_num;
    uint32_t schema_page_num; // page that holds the full Schema
} CompactTableMeta;

typedef struct CatalogOnDisk {
    uint32_t num_tables;
    CompactTableMeta tables[MAX_TABLES];
} CatalogOnDisk;

// In-memory rich entry
...
typedef struct Table {
    ...
    // Working context for the currently selected table (switched per-statement)
    uint32_t root_page_num;
    Schema schema;

    // Transaction state (database-wide for simplicity)
    bool in_transaction;
...
typedef struct MetaPage {
    uint32_t magic;
    CatalogOnDisk catalog;
} MetaPage;

static void catalog_save(Table* table) {
    MetaPage* meta = (MetaPage*)pager_get_page(table->pager, 0);
    meta->magic = SCHEMA_MAGIC;
    memset(&meta->catalog, 0, sizeof(CatalogOnDisk));
    meta->catalog.num_tables = table->catalog.num_tables;
    for (uint32_t i = 0; i < MAX_TABLES; i++) {
        meta->catalog.tables[i].used = table->catalog.tables[i].used ? 1 : 0;
        if (table->catalog.tables[i].used) {
            strncpy(meta->catalog.tables[i].name, table->catalog.tables[i].name, MAX_NAME_LEN - 1);
            meta->catalog.tables[i].root_page_num = table->catalog.tables[i].root_page_num;
            meta->catalog.tables[i].schema_page_num = table->catalog.tables[i].schema_page_num;
            // Persist the full Schema onto its dedicated page
            void* schema_page = pager_get_page(table->pager, table->catalog.tables[i].schema_page_num);
            memcpy(schema_page, &table->catalog.tables[i].schema, sizeof(Schema));
        }
    }
}

static int catalog_find(const Catalog* cat, const char* name) {
    for (uint32_t i = 0; i < MAX_TABLES; i++) {
        if (cat->tables[i].used && strcasecmp_custom(cat->tables[i].name, name) == 0) {
            return (int)i;
        }
    }
    return -1;
...
static int catalog_alloc_slot(Catalog* cat) {
    for (uint32_t i = 0; i < MAX_TABLES; i++) {
        if (!cat->tables[i].used) {
            return (int)i;
        }
    }
    return -1;
...
// Switch the Table's working context to the named table. Returns false if not found.
static bool table_select(Table* table, const char* name) {
    int idx = catalog_find(&table->catalog, name);
    if (idx < 0) return false;
    table->root_page_num = table->catalog.tables[idx].root_page_num;
    table->schema = table->catalog.tables[idx].schema;
    return true;
}

// After mutating schema/root of the working table, write it back into the catalog.
static void table_sync_working(Table* table, const char* name) {
    int idx = catalog_find(&table->catalog, name);
    if (idx < 0) return;
    table->catalog.tables[idx].root_page_num = table->root_page_num;
    table->catalog.tables[idx].schema = table->schema;
    catalog_save(table);
}
...
Table* table_open(const char* filename) {
    ...
    memset(&table->catalog, 0, sizeof(Catalog));
    ...
    if (table->pager->num_pages == 0) {
        // Fresh database: only meta page exists so far
        MetaPage* meta = (MetaPage*)pager_get_page(table->pager, 0);
        meta->magic = SCHEMA_MAGIC;
        memset(&meta->catalog, 0, sizeof(CatalogOnDisk));
    } else {
        ...
        if (meta->magic == SCHEMA_MAGIC) {
            table->catalog.num_tables = meta->catalog.num_tables;
            for (uint32_t i = 0; i < MAX_TABLES; i++) {
                if (meta->catalog.tables[i].used) {
                    table->catalog.tables[i].used = true;
                    strncpy(table->catalog.tables[i].name, meta->catalog.tables[i].name, MAX_NAME_LEN - 1);
                    table->catalog.tables[i].root_page_num = meta->catalog.tables[i].root_page_num;
                    table->catalog.tables[i].schema_page_num = meta->catalog.tables[i].schema_page_num;
                    // Load full Schema from its page
                    void* schema_page = pager_get_page(table->pager, meta->catalog.tables[i].schema_page_num);
                    memcpy(&table->catalog.tables[i].schema, schema_page, sizeof(Schema));
                }
            }
        } else {
            // Legacy or corrupt: start with empty catalog
            memset(&table->catalog, 0, sizeof(Catalog));
            meta->magic = SCHEMA_MAGIC;
            memset(&meta->catalog, 0, sizeof(CatalogOnDisk));
        }
...
PrepareResult prepare_statement(TokenList* list, Table* table, Statement* statement) {
    memset(statement, 0, sizeof(Statement));
    if (list->count == 0 || peek_token(list).kind == TOKEN_END) {
        return PREPARE_SYNTAX_ERROR;
    }

    // Default schema pointer used by WHERE / value parsing. Will be updated
    // once we know which table the statement targets.
    ...
    if (strcasecmp_custom(first.text, "insert") == 0) {
        ...
        strncpy(statement->table_name, tbl.text, MAX_NAME_LEN - 1);
        statement->table_name[MAX_NAME_LEN - 1] = '\0';

        int tidx = catalog_find(&table->catalog, statement->table_name);
        if (tidx < 0) {
            // Defer the "table does not exist" error to execute time so prepare
            // still returns PREPARE_SUCCESS for a clean message path.
            // For value parsing we still need a schema; use empty one.
            schema = &table->schema;
        } else {
            schema = &table->catalog.tables[tidx].schema;
        }
    ...
    // SELECT [* | COUNT(*)] FROM <table> [WHERE <expr>] [LIMIT n]
    if (strcasecmp_custom(first.text, "select") == 0) {
        ...
        PrepareResult where_res = parse_where_clause(list, schema, statement);
        if (where_res != PREPARE_SUCCESS) return where_res;

        // Optional LIMIT n
        if (match_token(list, "limit")) {
            Token lim = advance_token(list);
            if (lim.kind != TOKEN_NUMBER) return PREPARE_SYNTAX_ERROR;
            char* endptr;
            long lim_val = strtol(lim.text, &endptr, 10);
            if (*endptr != '\0' || lim_val < 0) return PREPARE_SYNTAX_ERROR;
            statement->has_limit = true;
            statement->limit = (uint32_t)lim_val;
        }
...
ExecuteResult execute_select(Statement* statement, Table* table) {
    ...
    while (!cursor->end_of_table) {
        ...
        if (row_matches_where(&row, statement, &table->schema)) {
            ...
            // Stop early if LIMIT has been reached (and not a COUNT query)
            if (statement->has_limit && !statement->is_count && match_count >= statement->limit) {
                break;
            }
        }
        cursor_advance(cursor);
    ...
    if (statement->is_count) {
        // For COUNT we still report the full match count (LIMIT does not apply to COUNT)
        printf("%u row(s).\n", match_count);
    }
    return EXECUTE_SUCCESS;
}
...
ExecuteResult execute_drop(Statement* statement, Table* table) {
    int idx = catalog_find(&table->catalog, statement->table_name);
    if (idx < 0) {
        printf("Error: table \"%s\" does not exist.\n", statement->table_name);
        return EXECUTE_NOT_FOUND;
    }

    // Mark slot free. Pages belonging to the dropped table are left orphaned
    // (a full free-list would be needed for perfect reclamation).
    table->catalog.tables[idx].used = false;
    memset(&table->catalog.tables[idx].schema, 0, sizeof(Schema));
    table->catalog.tables[idx].root_page_num = 0;
    table->catalog.tables[idx].schema_page_num = 0;
    if (table->catalog.num_tables > 0) table->catalog.num_tables--;

    // Clear working context if it pointed at the dropped table
    if (strcasecmp_custom(table->schema.table_name, statement->table_name) == 0) {
        memset(&table->schema, 0, sizeof(Schema));
        table->root_page_num = 0;
    }
...
ExecuteResult execute_create(Statement* statement, Table* table) {
    if (catalog_find(&table->catalog, statement->table_name) >= 0) {
        printf("Error: table \"%s\" already exists.\n", statement->table_name);
        return EXECUTE_DUPLICATE_KEY; // reuse for simplicity
    }

    int slot = catalog_alloc_slot(&table->catalog);
    if (slot < 0) {
        printf("Error: maximum number of tables (%d) reached.\n", MAX_TABLES);
        return EXECUTE_TABLE_FULL;
    }

    // Allocate a dedicated schema page, then the B+tree root page
    uint32_t schema_page = get_unused_page_num(table);
    pager_get_page(table->pager, schema_page); // force allocation

    uint32_t root_page = get_unused_page_num(table);
    void* root_node = pager_get_page(table->pager, root_page);
    initialize_leaf_node(root_node);
    set_node_root(root_node, true);

    TableMeta* meta = &table->catalog.tables[slot];
    meta->used = true;
    strncpy(meta->name, statement->table_name, MAX_NAME_LEN - 1);
    meta->name[MAX_NAME_LEN - 1] = '\0';
    meta->root_page_num = root_page;
    meta->schema_page_num = schema_page;
    meta->schema = statement->created_schema;
    // Ensure schema knows its own name
    strncpy(meta->schema.table_name, statement->table_name, MAX_NAME_LEN - 1);
    meta->schema.table_name[MAX_NAME_LEN - 1] = '\0';
    meta->schema.has_schema = true;

    table->catalog.num_tables++;
    catalog_save(table);

    // Also set as working context
    table->root_page_num = root_page;
    table->schema = meta->schema;

    printf("CREATE TABLE (%u columns configured)\n", meta->schema.num_columns);
    return EXECUTE_SUCCESS;
}

ExecuteResult execute_statement(Statement* statement, Table* table) {
    // For DML statements, switch working context to the named table.
    if (statement->type == STATEMENT_INSERT || statement->type == STATEMENT_SELECT ||
        ...
        if (!table_select(table, statement->table_name)) {
            printf("Error: table \"%s\" does not exist.\n", statement->table_name);
            return EXECUTE_NOT_FOUND;
        }
    }
    ...
    switch (statement->type) {
        ...
        case STATEMENT_UPDATE: {
            ExecuteResult r = execute_update(statement, table);
            // PK changes or other mutations may have updated the working schema/root
            table_sync_working(table, statement->table_name);
            return r;
        }
...
void print_help(void) {
    printf("SQL Commands:\n"
           ...
           "  SELECT * FROM <name> [WHERE <expr>] [LIMIT n];\n"
           "  SELECT COUNT(*) FROM <name> [WHERE <expr>];\n"
           "  UPDATE <name> SET <col> = <val> WHERE <expr>;\n"
           "  DELETE FROM <name> WHERE <expr>;\n"
           "  (WHERE supports =, !=, <>, <, >, <=, >=, AND, OR, and parentheses ())\n"
           "  BEGIN; | COMMIT; | ROLLBACK;\n"
           "  (Multiple tables are supported — always specify the table name.)\n"
           "Meta commands:\n"
           "  \\q or .exit      quit the shell\n"
           "  \\d or .btree     print B+tree structure for every table\n"
           "  \\s or .schema    print schema of every table\n"
...
void print_one_schema(const TableMeta* meta) {
    printf("Table \"%s\" (root page %u)\n", meta->name, meta->root_page_num);
    printf(" %-4s %-20s %-12s %-8s %s\n", "Idx", "Column", "Type", "Size", "Constraints");
    printf(" ---- -------------------- ------------ -------- -----------\n");
    for (uint32_t i = 0; i < meta->schema.num_columns; i++) {
        const ColumnDef* col = &meta->schema.columns[i];
        char type_buf[32];
        if (col->type == DATA_TYPE_INT) {
            strcpy(type_buf, "INT");
        } else {
            snprintf(type_buf, sizeof(type_buf), "VARCHAR(%u)", col->length);
        }
        printf(" %-4u %-20s %-12s %-8u %s\n",
               i,
               col->name,
               type_buf,
               col->size,
    ...
    printf("Row size: %u bytes\n\n", meta->schema.row_size);
}

void print_schema(const Table* table) {
    uint32_t found = 0;
    for (uint32_t i = 0; i < MAX_TABLES; i++) {
        if (table->catalog.tables[i].used) {
            print_one_schema(&table->catalog.tables[i]);
            found++;
        }
    }
    if (found == 0) {
        printf("No tables defined. Use CREATE TABLE first.\n");
    } else {
        printf("%u table(s).\n", found);
    }
}
...
MetaCommandResult do_meta_command(const char* input, Table* table) {
    if (strcmp(input, ".exit") == 0 || strcmp(input, "\\q") == 0) {
        ...
        for (uint32_t i = 0; i < MAX_TABLES; i++) {
            if (table->catalog.tables[i].used) {
                any = true;
                printf("Tree for table \"%s\":\n", table->catalog.tables[i].name);
                // Temporarily switch working context so print_tree uses the right schema/row_size
                uint32_t saved_root = table->root_page_num;
                Schema saved_schema = table->schema;
                table->root_page_num = table->catalog.tables[i].root_page_num;
                table->schema = table->catalog.tables[i].schema;
                print_tree(table, table->catalog.tables[i].root_page_num, 0);
                table->root_page_num = saved_root;
    ...
    } else if (strcmp(input, ".schema") == 0 || strcmp(input, "\\s") == 0) {
        print_schema(table);
        return META_COMMAND_SUCCESS;
...
int main(int argc, char* argv[]) {
    ...
    while (true) {
        ...
        Statement statement;
        PrepareResult prep_res = prepare_statement(&tokens, table, &statement);
        ...
        switch (exec_res) {
            ...
            case EXECUTE_DUPLICATE_KEY:
                // Specific message already printed by execute_insert / execute_create
                break;
            case EXECUTE_NOT_FOUND:
                // Specific message already printed by execute_drop / table lookup
                break;
