...
constexpr size_t COLUMN_NAME_SIZE = 32;
constexpr size_t COLUMN_DEPARTMENT_SIZE = 32;
constexpr size_t COLUMN_CITY_SIZE = 32;

struct Row {
    uint32_t id{0};
    char name[COLUMN_NAME_SIZE + 1]{0};
    int32_t salary{0};
    char department[COLUMN_DEPARTMENT_SIZE + 1]{0};
    char city[COLUMN_CITY_SIZE + 1]{0};
};

constexpr uint32_t ID_SIZE = sizeof(uint32_t);
constexpr uint32_t NAME_SIZE = COLUMN_NAME_SIZE + 1;
constexpr uint32_t SALARY_SIZE = sizeof(int32_t);
constexpr uint32_t DEPARTMENT_SIZE = COLUMN_DEPARTMENT_SIZE + 1;
constexpr uint32_t CITY_SIZE = COLUMN_CITY_SIZE + 1;
constexpr uint32_t ID_OFFSET = 0;
constexpr uint32_t NAME_OFFSET = ID_OFFSET + ID_SIZE;
constexpr uint32_t SALARY_OFFSET = NAME_OFFSET + NAME_SIZE;
constexpr uint32_t DEPARTMENT_OFFSET = SALARY_OFFSET + SALARY_SIZE;
constexpr uint32_t CITY_OFFSET = DEPARTMENT_OFFSET + DEPARTMENT_SIZE;
constexpr uint32_t ROW_SIZE = ID_SIZE + NAME_SIZE + SALARY_SIZE + DEPARTMENT_SIZE + CITY_SIZE;
...
// ============================================================================
//
// A WHERE clause is parsed into a small expression tree instead of a flat
// set of fields. This is what lets "WHERE a = 1 AND (b = 2 OR c = 3)" be
// represented at all: each AND/OR node has two Expr children, which may
// themselves be AND/OR nodes or leaf comparisons. Evaluating a row against
// the WHERE clause is then just a recursive walk of this tree.

// Row is already fully defined above; ComparisonExpr::evaluate operates on it directly.

struct Expr {
    virtual ~Expr() = default;
    // Returns whether `row` satisfies this (sub)expression.
    virtual bool evaluate(const Row& row) const = 0;
};

// A literal value on the right-hand side of a comparison, e.g. `1` or `'bob'`.
struct Literal {
...
// Leaf node: `<column> <op> <literal>`, e.g. `id >= 5` or `name = 'Alice'`.
struct ComparisonExpr : Expr {
    std::string column;  // "id" | "name" | "salary" | "department" | "city"
    ...
    bool evaluate(const Row& row) const override {
        int cmp = 0;
        if (column == "id") {
            uint32_t v = value.int_value;
            cmp = (row.id > v) - (row.id < v);
        } else if (column == "salary") {
            int32_t v = static_cast<int32_t>(value.int_value);
            cmp = (row.salary > v) - (row.salary < v);
        } else if (column == "name") {
            int c = std::strcmp(row.name, value.str_value.c_str());
            cmp = (c > 0) - (c < 0);
        } else if (column == "department") {
            int c = std::strcmp(row.department, value.str_value.c_str());
            cmp = (c > 0) - (c < 0);
        } else {  // "city"
            int c = std::strcmp(row.city, value.str_value.c_str());
            cmp = (c > 0) - (c < 0);
        }
...
// Interior node: `<left> AND <right>` or `<left> OR <right>`.
struct LogicalExpr : Expr {
    ...
    bool evaluate(const Row& row) const override {
        if (op == LogicOp::AND) return left->evaluate(row) && right->evaluate(row);
...
struct Statement {
    ...
    // WHERE clause: null means "no WHERE clause, match everything."
    ...
    bool set_name{false};
    bool set_salary{false};
    bool set_department{false};
    bool set_city{false};
    std::string set_name_value;
    int32_t set_salary_value{0};
    std::string set_department_value;
    std::string set_city_value;
};
...
void print_row(const Row* row) {
    std::cout << "(" << row->id << ", " << row->name << ", " << row->salary
...
void serialize_row(const Row* source, void* destination) {
    ...
    std::memcpy(node_ptr(destination, NAME_OFFSET), &(source->name), NAME_SIZE);
    std::memcpy(node_ptr(destination, SALARY_OFFSET), &(source->salary), SALARY_SIZE);
    std::memcpy(node_ptr(destination, DEPARTMENT_OFFSET), &(source->department), DEPARTMENT_SIZE);
    std::memcpy(node_ptr(destination, CITY_OFFSET), &(source->city), CITY_SIZE);
}
...
void deserialize_row(const void* source, Row* destination) {
    ...
    std::memcpy(&(destination->name), node_ptr(const_cast<void*>(source), NAME_OFFSET), NAME_SIZE);
    std::memcpy(&(destination->salary), node_ptr(const_cast<void*>(source), SALARY_OFFSET), SALARY_SIZE);
    std::memcpy(&(destination->department), node_ptr(const_cast<void*>(source), DEPARTMENT_OFFSET), DEPARTMENT_SIZE);
    std::memcpy(&(destination->city), node_ptr(const_cast<void*>(source), CITY_OFFSET), CITY_SIZE);
}
...
private:
    ...
    // ---- WHERE clause: recursive descent over the grammar --------------
    ...
    //
    // Parentheses are fully supported, e.g.
    //   WHERE a = 1 AND (b = 2 OR c = 3)
    // Left-association is handled by looping at each precedence level.
    ...
    PrepareResult parse_comparison(std::unique_ptr<Expr>& out) {
        ...
        if (col_name != "id" && col_name != "name" && col_name != "salary" &&
            col_name != "department" && col_name != "city") {
        ...
        if (col_name == "id" || col_name == "salary") {
            cmp->value.is_string = false;
            try {
                size_t consumed = 0;
                long id_val = std::stol(val.text, &consumed);
                if (consumed != val.text.size()) return PrepareResult::SYNTAX_ERROR;
                if (col_name == "id" && id_val < 0) return PrepareResult::NEGATIVE_ID;
                cmp->value.int_value = static_cast<uint32_t>(id_val);
            } catch (...) {
                return PrepareResult::SYNTAX_ERROR;
            }
        } else {
    ...
    PrepareResult parse_where_clause(Statement& statement) {
        if (!match("WHERE")) return PrepareResult::SUCCESS;  // no WHERE at all
...
public:
    ...
    PrepareResult prepare_statement(Statement& statement) {
        ...
        // INSERT INTO <table> [(cols)] VALUES (id, 'name', salary, 'department', 'city')
        if (first == "insert") {
            advance();
            match("into");
            advance(); // consume table name

            // Optional column specifier
            if (peek().text == "(") {
            ...
            if (peek().text == ",") advance();
            Token salary_tok = advance();
            if (peek().text == ",") advance();
            Token dept_tok = advance();
            if (peek().text == ",") advance();
            Token city_tok = advance();
            ...
            try {
                ...
                salary = std::stol(salary_tok.text);
            } catch (...) {
                return PrepareResult::SYNTAX_ERROR;
            }

            if (id < 0) return PrepareResult::NEGATIVE_ID;
            if (name_tok.text.length() > COLUMN_NAME_SIZE) return PrepareResult::STRING_TOO_LONG;
            if (dept_tok.text.length() > COLUMN_DEPARTMENT_SIZE) return PrepareResult::STRING_TOO_LONG;
            if (city_tok.text.length() > COLUMN_CITY_SIZE) return PrepareResult::STRING_TOO_LONG;

            statement.type = StatementType::INSERT;
            statement.row_to_insert.id = static_cast<uint32_t>(id);
            std::strncpy(statement.row_to_insert.name, name_tok.text.c_str(), COLUMN_NAME_SIZE);
            statement.row_to_insert.name[COLUMN_NAME_SIZE] = '\0';
            statement.row_to_insert.salary = static_cast<int32_t>(salary);
            std::strncpy(statement.row_to_insert.department, dept_tok.text.c_str(), COLUMN_DEPARTMENT_SIZE);
            statement.row_to_insert.department[COLUMN_DEPARTMENT_SIZE] = '\0';
            std::strncpy(statement.row_to_insert.city, city_tok.text.c_str(), COLUMN_CITY_SIZE);
            statement.row_to_insert.city[COLUMN_CITY_SIZE] = '\0';
        ...
        // OR point update syntax: UPDATE <id> <name> <salary> <department> <city>
        if (first == "update") {
            advance();
            ...
            if (to_lower(second.text) == "set" || (to_lower(peek().text) == "set")) {
                ...
                while (cursor < tokens.size() && to_lower(peek().text) != "where" && peek().text != ";") {
                    ...
                    std::string col_name = to_lower(col.text);
                    if (col_name == "name") {
                        if (val.text.length() > COLUMN_NAME_SIZE) return PrepareResult::STRING_TOO_LONG;
                        statement.set_name = true;
                        statement.set_name_value = val.text;
                    } else if (col_name == "salary") {
                        try {
                            statement.set_salary_value = static_cast<int32_t>(std::stol(val.text));
                            statement.set_salary = true;
                        } catch (...) {
                            return PrepareResult::SYNTAX_ERROR;
                        }
                    } else if (col_name == "department") {
                        if (val.text.length() > COLUMN_DEPARTMENT_SIZE) return PrepareResult::STRING_TOO_LONG;
                        statement.set_department = true;
                        statement.set_department_value = val.text;
                    } else if (col_name == "city") {
                        if (val.text.length() > COLUMN_CITY_SIZE) return PrepareResult::STRING_TOO_LONG;
                        statement.set_city = true;
                        statement.set_city_value = val.text;
                    } else {
                        return PrepareResult::SYNTAX_ERROR;
                    }
                }

                if (!statement.set_name && !statement.set_salary &&
                    !statement.set_department && !statement.set_city) {
                    return PrepareResult::SYNTAX_ERROR;
                }

                return parse_where_clause(statement);
            } else {
                // Classic point form: UPDATE <id> <name> <salary> <department> <city>
                Token name_tok = advance();
                Token salary_tok = advance();
                Token dept_tok = advance();
                Token city_tok = advance();
...
bool row_matches_where(const Row* row, const Statement* statement) {
    if (!statement->where) return true;  // no WHERE clause: everything matches
    return statement->where->evaluate(*row);
}
    ...
    while (!cursor->end_of_table) {
        ...
        if (row_matches_where(&row, statement)) {
            if (statement->set_name) {
                std::strncpy(row.name, statement->set_name_value.c_str(), COLUMN_NAME_SIZE);
                row.name[COLUMN_NAME_SIZE] = '\0';
            }
            if (statement->set_salary) {
                row.salary = statement->set_salary_value;
            }
            if (statement->set_department) {
                std::strncpy(row.department, statement->set_department_value.c_str(), COLUMN_DEPARTMENT_SIZE);
                row.department[COLUMN_DEPARTMENT_SIZE] = '\0';
            }
            if (statement->set_city) {
                std::strncpy(row.city, statement->set_city_value.c_str(), COLUMN_CITY_SIZE);
                row.city[COLUMN_CITY_SIZE] = '\0';
            }
...
ExecuteResult execute_delete(Statement* statement, Table* table) {
    if (!statement->where && statement->target_id != 0) {
...
void print_help() {
    std::cout << "SQL Commands:\n"
              ...
              << "  SELECT COUNT(*) FROM employees [WHERE department = 'SE'];\n"
