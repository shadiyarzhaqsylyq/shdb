...
// Leaf node: `<column> <op> <literal>`, e.g. `id >= 5` or `username = 'bob'`.
struct ComparisonExpr : Expr {
    std::string column;  // "id" | "username" | "email"
    ...
    bool evaluate(const Row& row) const override {
        ...
        if (column == "id") {
            ...
            int c = std::strcmp(row.username, value.str_value.c_str());
            cmp = (c > 0) - (c < 0);
        } else {  // "email"
            int c = std::strcmp(row.email, value.str_value.c_str());
            cmp = (c > 0) - (c < 0);
        }
...
private:
    ...
    //
    // Each level only knows about the level below it, which is what makes
    // this "recursive descent": parse_or_expr descends into parse_and_expr,
    // which descends into parse_comparison. Left-association is handled by
    // looping at each level rather than recursing on the left operand.
