...
// The schema struct (prefixed with a magic number) must fit inside a single
// page, since it is persisted to page SCHEMA_PAGE_NUM.
_Static_assert(sizeof(uint32_t) + sizeof(Schema) <= PAGE_SIZE,
               "Schema struct too large to persist in a single page");
...
// Serializes `schema` (prefixed with SCHEMA_MAGIC) into the dedicated schema
// page so it survives process restarts.
void write_schema_to_page(Pager* pager, const Schema* schema) {
    void* page = pager_get_page(pager, SCHEMA_PAGE_NUM);
    memset(page, 0, PAGE_SIZE);
    uint32_t magic = SCHEMA_MAGIC;
    memcpy(page, &magic, sizeof(uint32_t));
    memcpy((uint8_t*)page + sizeof(uint32_t), schema, sizeof(Schema));
}

// Reads a schema back from the dedicated schema page. Returns false (and
// leaves *out untouched) if the page doesn't contain a valid schema magic
// number, e.g. for a fresh/corrupt/legacy file.
bool read_schema_from_page(Pager* pager, Schema* out) {
    void* page = pager_get_page(pager, SCHEMA_PAGE_NUM);
    uint32_t magic = 0;
    memcpy(&magic, page, sizeof(uint32_t));
    if (magic != SCHEMA_MAGIC) {
        return false;
    }
    memcpy(out, (uint8_t*)page + sizeof(uint32_t), sizeof(Schema));
    return true;
}
...
Table* table_open(const char* filename) {
    ...
    // Page 0 is reserved for the persisted schema; the B+tree root lives at
    // page 1 onward.
    ...
    if (table->pager->num_pages == 0) {
        // Brand new database file: write out the default schema and
        // initialize an empty tree.
        table->schema = create_default_schema();
        write_schema_to_page(table->pager, &table->schema);
        void* root_node = pager_get_page(table->pager, table->root_page_num);
        initialize_leaf_node(root_node);
        set_node_root(root_node, true);
    } else {
        // Existing database file: read the persisted schema back. If the
        // file predates schema persistence (or is corrupt), fall back to
        // the default schema rather than crashing.
        if (!read_schema_from_page(table->pager, &table->schema)) {
            table->schema = create_default_schema();
        }
    }
    return table;
...
ExecuteResult execute_statement(Statement* statement, Table* table) {
    switch (statement->type) {
        ...
        case STATEMENT_CREATE: {
            ...
            // Persist the new schema immediately so it survives a restart.
            write_schema_to_page(table->pager, &table->schema);
            // The row layout just changed, so any existing tree data is no
            // longer valid under the new schema. Reinitialize an empty tree
            // at the root page.
            void* root_node = pager_get_page(table->pager, table->root_page_num);
            initialize_leaf_node(root_node);
            set_node_root(root_node, true);
