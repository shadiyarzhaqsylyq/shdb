...
typedef enum {
  ...
  EXECUTE_TABLE_FULL          // NEW: table has hit TABLE_MAX_PAGES
} ExecuteResult;
...
// A single INSERT can cascade a split at every level of the tree (leaf,
// then each internal ancestor, then a new root). Reserve enough headroom
// that a worst-case cascade never runs get_page() out of pages mid-insert.
...
void* get_page(Pager* pager, uint32_t page_num) {
  if (page_num >= TABLE_MAX_PAGES) {
    printf("Tried to fetch page number out of bounds. %d >= %d\n", page_num,
           TABLE_MAX_PAGES);
    printf("Table has reached its maximum size (%d pages).\n", TABLE_MAX_PAGES);
    exit(EXIT_FAILURE);
  }
...
// NEW: strict integer parse. atoi() silently returns 0 for garbage input
// ("abc" -> 0), which let bad ids slip through as valid. This rejects
// anything that isn't a full, well-formed integer.
PrepareResult parse_int_strict(const char* str, long* out_value) {
  if (str == NULL || *str == '\0') return PREPARE_SYNTAX_ERROR;
  char* endptr;
  errno = 0;
...
PrepareResult parse_where_clause(Statement* statement) {
  ...
  if (strcmp(column, "id") == 0) {
    ...
    long int_value;
    PrepareResult result = parse_int_strict(value, &int_value);
    if (result != PREPARE_SUCCESS) return result;
    if (int_value < 0) return PREPARE_NEGATIVE_ID;
    statement->where_int_value = (uint32_t)int_value;
  } else {
...
PrepareResult prepare_insert(InputBuffer* input_buffer, Statement* statement) {
  ...
  long id;
  PrepareResult id_result = parse_int_strict(id_string, &id);
  if (id_result != PREPARE_SUCCESS) return id_result;
...
PrepareResult prepare_delete(InputBuffer* input_buffer, Statement* statement) {
  ...
  long id;
  PrepareResult id_result = parse_int_strict(next, &id);
  if (id_result != PREPARE_SUCCESS) return id_result;
  if (id < 0) return PREPARE_NEGATIVE_ID;
